...
@jit(nopython=True)
def MC_update(state_arr,flag_arr,N):
...
@jit(nopython=True)
def get_data(tt,init_state,flag_arr,N,m_arr,m2_arr,m4_arr):
...
if __name__=='__main__':
    ...
    flag_arr=np.array([pos for pos in range(len(init_state)) if init_state[pos]!=-1])
    N=len(flag_arr)
    ...
    while True:
        k+=1
        get_data(tt,init_state,flag_arr,N,m_arr,m2_arr,m4_arr)
        y=np.zeros((3,760))
