...
def get_data2(init_state):
    ...
    for i in range(N_ensemble):
        ...
        for j in range(2*10**6):
            for k in range(N_sites):
                MC_update(state_arr)
        phi=get_largest_cluster(state_arr)
        m+=phi
        m2+=phi*phi
        m4+=phi*phi*phi*phi
    return m,m2,m4
...
if __name__=='__main__':
    ...
    data,k,Nens=get_k_Nens2(fname)

    #m,m2,m4=data[0],data[1],data[2]

    while True:
        k+=1
        data=data+get_data2(init_state)
        np.savetxt(f'data_{p}_{omega}_{L}.txt',data/N_ensemble/k)
