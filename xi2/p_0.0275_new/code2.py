...
@jit(nopython=True)
def get_data(tt,init_state,c_arr):
    for i in range(N_ensemble):
        state_arr=init_state.copy()
        t=0
        for j in range(660): #tt[760] ~ 10^5
            while(t<tt[j]):
                for k in range(N_sites):
                    MC_update(state_arr)
                t+=1
        for j in range(660,760):
            while(t<tt[j]):
                for k in range(N_sites):
                    MC_update(state_arr)
                t+=1

            corr(state_arr,c_arr)



@jit(nopython=True)
def get_data2(tt,init_state,m_arr,m2_arr,m4_arr):
    for i in range(N_ensemble):
        state_arr=init_state.copy()
        t=0
...
def get_k_Nens(fname):
    ...
    try:
        data=np.loadtxt(fname,max_rows=L)
...
if __name__=='__main__':
    ...
    while True:
        k+=1
        get_data(tt,init_state,c_arr)
        
        y=c_arr/(N_ensemble*k)
