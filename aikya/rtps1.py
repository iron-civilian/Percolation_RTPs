...
def run():
	...
	for en in range(ensemble):
		...
		for t in range(tmax):
			...
			vvmin += mmin*mmin/tmax/ensemble
			vvvvmin += mmin*mmin*mmin*mmin/tmax/ensemble
			vmin2 += mmin2/tmax/ensemble
			vvmin2 += mmin2*mmin2/tmax/ensemble
			vvvvmin2 += mmin2*mmin2*mmin2*mmin2/tmax/ensemble
			vmax += mmax/tmax /ensemble
			vvmax += mmax*mmax/tmax /ensemble
			vvvvmax += mmax*mmax*mmax*mmax/tmax /ensemble
		
	return vmax,vvmax,vvvvmax, vmin,vvmin,vvvvmin, vmin2,vvmin2,vvvvmin2, lattice
	
vmax,vvmax,vvvvmax, vmin,vvmin,vvvvmin, vmin2,vvmin2,vvvvmin2, lattice = run()
fname =f"2dataL{L}p{p}.csv"
file1 = open(fname, "a")  # append mode
data1 = str(p) + "," + str(omega) + "," + str(rho) + "," + str(vmax) + ","+str(vvmax) + ","+str(vvvvmax) + "," + str(vmin) + ","+str(vvmin) + ","+str(vvvvmin) + "," + str(vmin2)+","+str(vvmin2) + ","+str(vvvvmin2) + "\n"
file1.write(data1)
file1.close()
'''
