...
@jit(nopython=True)
def gdata(init_state):
    for i in range(N_sites*10**u):
        MC_update(init_state)
...
if __name__=='__main__':
    ...
    ti=time.time()
    gdata(init_state)
    np.savetxt(f'L{L}T{u}p{p}w{omega}.txt',init_state)
    tf=time.time()
    print(f"p={p},w={omega},u={u},t={(tf-ti)/60} mins")
