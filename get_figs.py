...
if __name__=='__main__':
    ...
    state_arr=np.loadtxt(f'state_arr_{L}_ordered.txt')
    ...
    while True:
        k+=1
        get_data(tt,state_arr,10)
        
        plt.imshow(state_arr.reshape((Lx,Ly)),cmap=custom_cmap, vmin=vmin, vmax=vmax)
        plt.title(f'#MC steps=10')
        plt.savefig('fig1.png')
        for i in range(11,1001):
            get_data(tt,state_arr,1)
            plt.imshow(state_arr.reshape((Lx,Ly)),cmap=custom_cmap, vmin=vmin, vmax=vmax)
            plt.title(f'#MC steps={i}')
            plt.savefig(f'fig{i-9}.png')
        break
